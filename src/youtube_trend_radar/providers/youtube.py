...
from youtube_trend_radar.resolution import STOPWORDS
...
def build_queries(candidate: Candidate, limit: int = 2) -> list[str]:
    ...
    entity_words = {word.lower() for word in WORD_RE.findall(entity)}
    distinctive: list[str] = []
    seen: set[str] = set()
    for word in WORD_RE.findall(candidate.title):
        lower = word.lower()
        if lower in STOPWORDS or lower in entity_words or lower in seen:
            continue
        seen.add(lower)
        distinctive.append(word)
    phrase = clean_text(" ".join([entity, *distinctive[:8]]).strip() or candidate.title, limit=100)
    item_types = {item.item_type for item in candidate.items}
    modifier = "tutorial" if any("repository" in value or "space" in value for value in item_types) else "explained"
    queries = [phrase]
    if limit > 1:
        queries.append(clean_text(f"{phrase} {modifier}", limit=100))
    ...
    *,
    ...
    if disabled or not bool(config.youtube.get("enabled", True)) or not key:
        ...
        for candidate in targets:
            queries = build_queries(candidate, int(config.youtube.get("queries_per_candidate", 2)))
            candidate.youtube = {
                "status": "disabled",
                "reason": reason,
                "queries": queries,
    ...
    for candidate in targets:
        queries = build_queries(candidate, queries_per_candidate)
